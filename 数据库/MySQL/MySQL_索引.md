...
- 非聚簇索引：索引数据和存储数据是分离的，就是将数据与索引分开存储，叶子节点包含索引字段值及指向数据页数据行的逻辑指针
- 二级索引：存储的是记录的主键，而不是数据存储的地址。唯一索引、普通索引、前缀索引等都是二级索引（辅助索引）

对于聚簇索引来说，他的非叶子节点上存储的是索引值,而1它的叶子节点上存储的是整行记录。  
...
联合索引不满足传统“最左匹配”时，MySQL 把索引的前导列枚举出来，分别进行索引范围扫描，从而间接利用后面的索引列。

MySQL 8.0.13版本中，对于range查询(什么是range后面会提到)，引入了索引跳跃扫描(Index Skip Scan)优化，支持不符合组合索引最左前缀原则条件下的SQL，依然能够使用组合索引，减少不必要的扫描。
...
-- 索引是：(gender, age)
```
现在查询：`SELECT * FROM user WHERE age = 20;`，按照传统最左匹配原则：
```text
索引：gender, age
       ↑
查询没有 gender
```
通常无法直接利用 `age` 进行普通的索引定位。传统情况下可能走：全表扫描；
...
WHERE gender = 1 AND age = 20;
```
然后分别利用索引：`(gender, age)` 这个联合索引进行查找；

所以它叫 Skip Scan：跳过联合索引的第一个列 `gender`，通过枚举它的不同值，间接使用后面的 `age`。

### 为什么这种方式可能有效？

关键在于：gender 的基数很低，这里 gender 只有两个值，只需要扫描两次

如果索引是：`(gender, age)`，那么索引内部大致是：
```text
gender=0
    age=18
    age=19
...
查询：`WHERE age = 20`，就可以：
```sql
跳到 gender=0 → 找 age=20
跳到 gender=1 → 找 age=20
```
而不是扫描整个索引。

### 与最左前缀匹配

只有当前导列基数较低、Skip Scan 成本低于其他执行计划时，优化器才可能选择它。但是，我们不能依赖他这个优化，建立索引的时候，还是优先把区分分度高的，查询频繁的字段放到联合索引的左边。
...
| 对比       | 索引下推 ICP                | 索引跳跃扫描 Skip Scan            |
| -------- | ----------------------- | --------------------------- |
| 解决什么问题   | 减少回表                    | 绕过前导索引列限制                   |
| 核心动作     | 先在索引中过滤                 | 枚举前导列值进行多次扫描                |
| 是否突破最左匹配 | 否                       | 可以在特定场景下间接利用后列              |
| 主要收益     | 减少回表次数                  | 减少扫描范围                      |
| 典型表现     | `Using index condition` | `Using index for skip scan` |

### 总结

Skip Scan 是优化器在特定情况下对“最左匹配原则”的一种执行层面的绕行，而不是改变了 B+Tree 的最左匹配规则
...
- using index：表示相应的查询操作中使用了覆盖索引，避免访问表的数据行。如果同时出现 using where，说明索引是用来执行索引键值的查找；如果没有同时出现 using where，说明索引是用来读取数据而非执行查找动作；
	- 覆盖索引：select 的数据列从索引中就能够取得，不必读取数据行，MySQL 可以利用索引返回 select 列表中的字段，而不必根据索引再次读取数据文件，换句话说查询列要被所建的索引覆盖。如果使用覆盖索引，要注意 select 列表只读取所需要的列，不要使用 select *，因为如果将所有字段一起做成索引会导致索引文件过大，查询性能下降。
