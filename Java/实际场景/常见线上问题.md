...
- 问题2：有一个系统A同时对接了两个系统B、C，B、C系统都会通知系统A更新某个表的某个字段，更新的where条件中有两个字段，两个字段都是单值索引，导致更新时存在死锁；根据两个条件查询时的执行计划type是`index_merge`
    ...
    explain update test set price = 2.66 where code='001';
    explain update test set price = 2.66 where parentCode = 'XS0001'

    如果两个索引扫描出来的记录顺序不同，就可能出现：
    T1：先锁 id=1，等待 id=2
    T2：先锁 id=2，等待 id=1
    所以这样并发更新就可能出现死索引
    ```
...
我之前在一个订单相关系统中遇到过一次比较典型的 MySQL 死锁问题。

当时的业务场景是 A 系统处理一批业务数据，在事务里面会调用 B 系统做进一步处理，B 系统处理完成之后，再通过回调通知 A 系统继续更新原来的业务数据。

当时比较大的问题是，A 系统这个事务持续时间比较长，因为中间包含了跨系统调用，所以数据库事务中的锁持有时间比较长。

上线以后，在并发比较高的情况下，偶尔出现 MySQL Deadlock，表现为部分事务更新失败。

当时我首先通过 MySQL 的 `SHOW ENGINE INNODB STATUS` 查看最近一次死锁信息，同时结合数据库慢日志和应用日志，把死锁涉及的事务和 SQL 对应起来。

最后发现，问题和我们的更新方式有关。

A 系统更新数据时使用的是二级索引条件，例如根据业务状态或者业务编号更新记录。InnoDB 在更新过程中，不仅会涉及二级索引记录，同时还需要处理聚簇索引，也就是主键索引对应的记录。

而当时两个并发事务获取锁的顺序并不一致：

事务 A 先持有了一部分二级索引相关的锁，然后继续获取主键记录上的锁；

事务 B 则先获取了主键记录上的锁，再去获取另外的索引记录。

最终形成了类似：

A 等 B 释放锁，B 又等 A 释放锁。

于是形成循环等待，InnoDB 检测到死锁之后，会主动回滚其中一个事务。

定位到问题以后，我们主要从几个方面进行了处理。

第一，尽量缩短事务范围，把调用 B 系统这种跨系统 RPC 操作移到事务之外，避免数据库事务长时间持有锁。

第二，对于数据库更新，尽量使用主键或者唯一索引进行定位，减少锁范围，并统一不同业务流程中的更新顺序。

第三，对这个核心流程增加了死锁异常的重试机制，因为 InnoDB 的死锁属于并发场景下可能发生的正常现象，业务上需要具备一定的容错能力。

另外，我们也对相关 SQL 和索引进行了优化，避免因为索引设计不合理导致锁定范围过大。

这次问题让我比较深的一个体会是，MySQL 死锁不一定是单条 SQL 本身有问题，很多时候是多个事务并发执行时，因为锁的获取顺序不同形成了循环等待。所以排查的时候，不能只看 SQL，而是要结合事务边界、索引、锁类型以及多个事务的执行顺序一起分析。

> 这个问题本质上是一个长事务导致锁持有时间过长，再叠加多个事务更新数据时锁获取顺序不一致，最终形成循环等待。我的处理思路是先通过 InnoDB 死锁日志还原事务和锁的关系，然后缩短事务边界、统一更新顺序、优化索引，最后增加死锁重试和幂等机制。

## 为什么二级索引会导致死锁？

二级索引本身不会直接导致死锁。使用二级索引进行更新时，InnoDB 需要同时处理二级索引记录和聚簇索引记录。如果多个事务并发更新多条相关数据，并且获取这些记录锁的顺序不一致，就可能形成循环等待，从而产生死锁。
...
- [常见线上问题排查](https://wx.zsxq.com/group/48418884588288/topic/185425252544152)
