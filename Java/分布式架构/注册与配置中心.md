...
- 编辑：`zoo.cfg`，修改：`dataDir=/user/local/zookeeper/zkdata`
- 配置输入三台集群的IP，端口等信息，一般格式是：`server.id=host:port:port`
	```
	server.1=192.168.89.156:2888:3888
	server.2=192.168.89.157:2888:3888
	server.3=192.168.89.158:2888:3888
	```
