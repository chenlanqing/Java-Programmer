...
说明：[偏向锁从 JDK 15 开始默认关闭并废弃](https://openjdk.org/jeps/374)，JDK 18 中被移除。
- JDK 15：偏向锁被废弃并默认关闭，可通过 -XX:+UseBiasedLocking 手动开启。
- JDK 18：相关 JVM 参数被标记为过时，偏向锁实现基本被移除。
- JDK 19 及以后：UseBiasedLocking 等参数不再可用；  
核心原因：收益越来越小，但实现和维护成本很高，锁撤销还可能带来额外开销
