...
2.Java 内存区域
	...
	2.2.内存区域:Java 虚拟机规范将 JVM 所管理的内存分为以下几个运行时数据区:
		...
		2.2.3.本地方法栈:使用到的本地操作系统（Native）方法服务
			与虚拟机栈发挥的作用类似;
		2.2.4.Java 堆:Java 虚拟机所管理的内存中最大的一块，它是所有线程共享的一块内存区域。
...
3.内存溢出与内存泄漏:
	...
	3.6.Java8 移出永久代:在JDK1.7中,已经把原本放在永久代的字符串常量池移出,放在堆中,因为使用永久代来实现方法区不是个好主意
		很容易遇到内存溢出的问题. 我们通常使用 PermSize 和 MaxPermSize 设置永久代的大小, 这个大小就决定了永久代的上限, 但是我们
		不是总是知道应该设置为多大的, 如果使用默认值容易遇到 OOM 错误.
		建议JVM的实现中将类的元数据放入 native memory, 将字符串池和类的静态变量放入java堆中. 这样可以加载多少类的元数据就不
		在由MaxPermSize控制, 而由系统的实际可用空间来控制
		更深层的原因还是要合并HotSpot和JRockit的代码, JRockit 从来没有一个叫永久代的东西, 但是运行良好, 也不需要开发运维人员设
		置这么一个永久代的大小.
4.对象实例化分析:
...
11.Java 垃圾收集机制 :// http://www.importnew.com/20129.html
	F:\Knowledge\Java\JavaSE\Java-JVM\Java-GC垃圾回收机制.java
