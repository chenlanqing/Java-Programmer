...
二.Java 内存模型基础:	http://www.infoq.com/cn/articles/java-memory-model-1
1.并发编程模型分类:
...
2.Java 的并发采用的是共享内存模型,Java 线程之间的通信总是隐式进行,整个通信过程对程序员完全透明
3.Java 内存模型的抽象
...
4.重排序:
	...
	===========================================================================================================================	
			①.StoreLoad Barriers 是一个“全能型”的屏障，它同时具有其他三个屏障的效果,它同时具有其他三个屏障的效果
...
三.JMM-重排序:	http://www.infoq.com/cn/articles/java-memory-model-2
...
2.as-if-serial语义:不管怎么重排序(编译器和处理器为了提高并行度),(单线程)程序的执行结果不能被改变.
...
1.数据竞争与顺序一致性保证:
	...
	1.2.JMM 对正确同步的多线程程序的内存一致性做了如下保证
		...
		顺序一致性内存模型中的执行结果相同
2.顺序一致性内存模型:顺序一致性内存模型是一个理论参考模型，JMM和处理器内存模型在设计时通常会把顺序一致性内存模型作为参照
	2.1.两大特征:
...
3.1.volatile 关键字用于表示可以被多个线程异步修改的成员变量,强制从公共堆栈中取得变量的值,不是从私有线程获取
		...
		缓存一致性问题:	
	3.2.volatile关键字的两层语义:一旦一个共享变量(类的成员变量、类的静态成员变量)被volatile修饰之后,那么就具备了两层语义:		
...
4.volatile内存语义的实现:
	...
	4.3.为了实现volatile的内存语义，编译器在生成字节码时，会在指令序列中插入内存屏障来禁止特定类型的处理器重排序:
		...
		(1).在每个volatile写操作的前面插入一个 StoreStore 屏障:
			报证在volatile写之前,其前面的所有普通写操作已经对任意处理器可见了
...
3.锁内存语义的实现:借助 ReentrantLock 的源代码，来分析锁内存语义的具体实现机制
...
七.final:(F:\Knowledge\Java\Java细节知识点.java)
