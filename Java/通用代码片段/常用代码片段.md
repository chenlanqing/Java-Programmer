...
```java
private static Class<?> getCommandClazz(Class<?> handlerImpl, Class<?> handlerInterface) {
    Type[] genericInterfaces = handlerImpl.getGenericInterfaces();
    boolean find = false;
    if (genericInterfaces.getClass().isAssignableFrom(ParameterizedType[].class)) {
        for (Type genericInterface : genericInterfaces) {
            ParameterizedType parameterizedType = (ParameterizedType) genericInterface;
            if (parameterizedType.getRawType() != handlerInterface) {
                continue;
            }
            ...
            try {
                ...
                log.error("load handler ex, handler={},parameterizedType={}", handlerImpl.getName(), parameterizedType.getRawType());
                throw e;
            }
        }
    }
    // 父类找
    // 格式xxHandler extends AbsXX<cmd,resp>, 其中AbsXX<T,K> implements CommandHandler<T,K>
    boolean implCommandHandler = false;
    for (Class<?> anInterface : handlerImpl.getSuperclass().getInterfaces()) {
        if (anInterface == handlerInterface) {
            implCommandHandler = true;
            break;
        }
    }
    if (implCommandHandler) {
        Type genericClass = handlerImpl.getGenericSuperclass();
    ...
    log.error("load handler fail, cannt find command class , handler={}", handlerImpl.getName());
