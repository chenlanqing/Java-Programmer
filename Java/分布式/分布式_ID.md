...
### 3.4、[生成有序的UUID](https://github.com/cowtowncoder/java-uuid-generator)
...
- 唯一性：订单号必须保证唯一性，否则会出现订单冲突和数据不一致等问题。可以使用一些常见的唯一性生成算法，例如UUID、Snowflake等。
...
| 业务类型（红色） | 雪花算法ID（绿色） | 分表结果（橙色） |
| ---- | ---- | ---- |
| 10 | 109525038028500006 | 0128 |

完整拼接订单号：`10 109525038028500006 0128`
1. **业务类型（10）**：标识当前业务线，用于区分不同业务订单
2. **雪花算法**：全局唯一递增ID，保证订单唯一性
3. **分表结果（0128）**：基因法分表字段，用于分库分表路由
