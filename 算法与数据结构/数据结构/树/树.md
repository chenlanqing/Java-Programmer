...
父节点、子节点、叶子节点、兄弟节点，除了这些概念，还有3个相似概念：
- 节点的高度（Height）：节点的高度=节点到叶子节点的最长路径；
...
（2）完全二叉树：一棵二叉树至多只有最下面的一层上的结点的度数可以小于2，并且最下层上的结点都集中在该层最左边的若干位置上，则此二叉树成为完全二叉树（可以使用数组来表示完全二叉树，通过数组下标来确定节点位置：如果节点 X 存储在数组中下标为 i 的位置，下标为 $2 * i$ 的位置存储的就是左子节点，下标为 $2 * i + 1$ 的位置存储的就是右子节点。反过来，下标为 $i/2$ 的位置存储就是它的父节点）
...
递归树是一种特殊的树结构，可以借助递归树来分析递归算法的时间复杂度，比如斐波那契数列数列，节点里的数字表示数据的规模，一个节点的求解可以分解为左右子节点两个问题的求解
...
为什么有了散列表还要有二分搜索树
- 第一，散列表中的数据是无序存储的，如果要输出有序的数据，需要先进行排序。而对于二分搜索树来说，我们只需要中序遍历，就可以在 O(n) 的时间复杂度内，输出有序的数据序列；
- 第二，散列表扩容耗时很多，而且当遇到散列冲突时，性能不稳定，尽管二分搜索树的性能不稳定，但是在工程中，我们最常用的平衡二分搜索树的性能非常稳定，时间复杂度稳定在 $O({\log}n)$
- 第三，笼统地来说，尽管散列表的查找等操作的时间复杂度是常量级的，但因为哈希冲突的存在，这个常量不一定比 logn 小，所以实际的查找速度可能不一定比 $O({\log}n)$ 快。加上哈希函数的耗时，也不一定就比平衡二分搜索树的效率高;
- 第四，散列表的构造比二分搜索树要复杂，需要考虑的东西很多。比如散列函数的设计、冲突解决办法、扩容、缩容等。平衡二分搜索树只需要考虑平衡性这一个问题，而且这个问题的解决方案比较成熟、固定
...
堆是基于完全二叉树实现的，在将一个数组调整成一个堆的时候，关键之一：是确定最后一个非叶子节点的序号，这个序号为`n/2-1`，n为数组的长度。但是为什么呢？
可以分两种情形考虑：
- ① 堆的最后一个非叶子节点若只有左孩子；
- ② 堆的最后一个非叶子节点有左右两个孩子；

完全二叉树的性质之一是：如果节点序号为`i`，在它的左孩子序号为`2*i+1`，右孩子序号为`2*i+2`。
- 对于`①`左孩子的序号为`n-1`，则`n-1 = 2*i-1`，推出`i=n/2-1`；
- 对于`②`左孩子的序号为`n-2`，在`n-2 = 2*i-1`，推出`i=(n-1)/2-1`；右孩子的序号为`n-1`，则`n-1=2*i+2`，推出`i=(n-1)/2-1`；

当完全二叉树最后一个节点是其父节点的左孩子时，树的节点数为偶数；当完全二叉树最后一个节点是其父节点的右孩子时，树的节点数为奇数；

根据java语法的特征，整数除不尽时向下取整，则若n为奇数时`(n-1)/2-1=n/2-1`。因此对于`②`最后一个非叶子节点的序号也是`n/2-1`；

> 上述推导过程是在索引从 0 开始的；

## 3、堆的实现

详细[MaxHeap.java](code/MaxHeap.java)
...
增加、查询的过程详细查看代码[Trie.java](code/Trie.java)
...
AVL树是最早被发明的自平衡二分搜索树。在AVL树中，任一节点对应的两棵子树的最大高度差为1，因此它也被称为高度平衡树。查找、插入和删除在平均和最坏情况下的时间复杂度都是 O(logN）。增加和删除元素的操作则可能需要借由一次或多次树旋转，以实现树的重新平衡
...
平衡二分搜索树：简称平衡二叉树。由前苏联的数学家 Adelse-Velskil 和 Landis 在 1962 年提出的高度平衡的二叉树，根据科学家的英文名也称为 AVL 树。它具有如下几个性质：
- 可以是空树；
...
![](image/2-3树-基本结构-示例.png)

上图中这颗树是一颗2-3树，其有如下特点：
- 如果一个节点中只有1个数据元素，比如50，其有最多只有两个孩子，且`左节点 < 节点 < 右节点`；
- 如果一个节点中有2个数据元素（A、B）比如（17、33），其最多只有三个孩子，那么`左节点 < A < 中间节点 < B < 右节点`

## 1.2、如何维持平衡

2-3树的添加有一个原则：永远不会添加到一个空子树中

![](image/2-3树-添加元素过程-1.png)

整体插入情况分为如下情况：

![](image/2-3树-添加元素过程-拆分.png)

## 2、红黑树定义

一种不严格的平衡二叉查找树；红黑树中的节点，一类被标记为黑色，一类被标记为红色。除此之外，一棵红黑树还需要满足这样几个要求：
- 根节点是黑色的；
