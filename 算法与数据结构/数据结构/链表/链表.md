...
- [剑指Offer-06](https://leetcode.cn/problems/cong-wei-dao-tou-da-yin-lian-biao-lcof/)
...
```java
public void reversePrintOrder(ListNode head) {
    ListNode p = head;
...
* 递归逆序打印链表
*/
public void recursiveReversePrintOrder(ListNode head) {
	if (head == null) {
		return;
	}
	recursiveReversePrintOrder(head.next);
...
public static ListNode reverseList(ListNode head){
	...
	// 翻转头节点与第二个节点的指向,让 2 的 next 指向 1
	node.next = head;
	// 此时的 head 节点为尾节点，next 需要指向 NULL  1 的 next 指向 null.
	head.next = null;
	// 把调整之后的链表返回。
	return newNode;
}
```
上面的写法是从后往前递归，那么从前往后递归呢？
```java
public static ListNode reverseList(ListNode head) {
...
慢指针slow走在后面，快指针fast走在前面探路，找到一个不重复的元素就告诉slow并让slow前进一步。这样当fast指针遍历完整个链表之后，将slow.next指针断开与后面重复元素的连接；
```java
...
public ListNode rotateRight(ListNode head, int k) {
    ...
    // 获取链表的长度
    int size = getLengthOfLinkedList(head);
    // 如果k超过链表的长度，取模运算
    if (k >= size) {
        k = k % size;
    }
    // 如果 k == 0，表示是链表长度的整数倍，直接返回头结点
    if (k == 0) {
        return head;
    }
    // 从头遍历，遍历到需要旋转节点的头一个节点，所以 index++ < size - k
    ListNode pre = head;
    int index = 1;
    while (index < size - k) {
        pre = pre.next;
    ...
    // 找到最后一个节点，需要将该节点指向head节点，这样旋转就完成了
...
- [约瑟夫环详细思路](https://www.cxyxiaowu.com/1159.html)
...
```java
public static void josephusCircle(int n, int start, int step) {
	...
	while (count < n - 1) {
		for (int j = start; j <= n + 1; j++) {
			...
			if (1 == arr[j]) { // 在未出圈的人数中计数
				num++;
				if (num == step) {// 此人数到m则出圈
					System.out.println("第【" + (j) + "】出列");
					count++; // 出圈人数加1
					num = 0;
					arr[j] = 0;// 出圈
				}
				if (count == n - 1) {// 如果出圈人数已经小于现有人数-1，则退出循环
					break;
				}
			}
		}
