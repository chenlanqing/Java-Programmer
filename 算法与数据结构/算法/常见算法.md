...
一致性哈希将整个哈希值空间组织成一个虚拟的圆环，如假设某哈希函数H的值空间为`0-2^32-1`（即哈希值是一个32位无符号整型），整个空间按顺时针方向组织。0和2^32-1在零点中方向重合；
...
虽然一致性 Hash 提升了稳定性，使节点的加入和退出不会造成大规模的数据迁移，但本质上 Hash 分片是一种静态的分片方式，必须要提前设定分片的最大规模，而且无法避免单一热点问题， 某一数据被海量并发请求后，不论如何进行 Hash，数据也只能存在一个节点上，这势必会带来热点请求问题

比如案例中的电商商品，如果某些商品卖得非常火爆，通过 Hash 分片的方式很难针对热点商品做单独的架构设计，如何解决单一热点问题？
...
- [二分查找实现](https://programmercarl.com/0704.%E4%BA%8C%E5%88%86%E6%9F%A5%E6%89%BE.html)
...
二分查找涉及的很多的边界条件，逻辑比较简单，例如到底是 `while(left < right)` 还是 `while(left <= right)`，到底是right = middle呢，还是要`right = middle - 1`呢？
...
- `if (nums[middle] > target)` right 要赋值为 `middle - 1`，因为当前这个`nums[middle]`一定不是target，那么接下来要查找的左区间结束下标位置就是 `middle - 1`，这样就形成了如下区间`[left, middle - 1]`
```java
...
### 3.6、在排序数组中查找元素的第一个和最后一个位置

- [统计一个数字在排序数组中出现的次数](../数据结构/数组/数组.md#14统计一个数字在排序数组中出现的次数)
...
- 3、如果二分查找成功，则 binarySearch 返回 nums 中值为 target 的一个下标。然后，通过左右滑动指针，来找到符合的区间
```java
public int[] searchRange(int[] nums, int target) {
    if (nums == null || nums.length == 0) {
        return new int[]{-1, -1};
...
- [大数据量处理面试题](https://www.yuque.com/zeovo-10k9s/lqwlrb/wwaqom2bv90lqloi?singleDoc=)
- [海量数据存储与处理](../../软件工程/软件设计/实际业务/各种业务.md#五海量数据存储与处理)
...
所谓的 Bit-map 就是用一个 bit 位来标记某个元素对应的 Value，而Key即是该元素。由于采用了Bit为单位来存储数据，因此在存储空间方面，可以大大节省
...
可以参考JDK中的 BitSet，或者谷歌：EWAHCompressedBitmap，其做了很多优化
```xml
<dependency>
...
也可以使用[Redis的Bitmap来实现](../../数据库/NoSQL/Redis/Redis_数据类型.md#6位图-bitmap)

### 1.3、[Roaring Bitmap](https://roaringbitmap.org/)

- [什么是Roaring Bitmap](https://mp.weixin.qq.com/s/8fqQFZudlbYJy4ezSqmveg)

Bitmap和Roaring Bitmap都为整数提供了一组数据结构，可以插入整数、检查整数的存在以及获取两组整数的交集和并集。Roaring Bitmap在集合操作性能的前提下，比Bitmap具有更好的压缩效果

**Roaring Bitmap 解决了什么问题？**

当集合稀疏时，Bitmap的压缩效果较差。假设您有一个空的Bitmap，向其中添加整数8,000,000。将发生以下情况：
- 首先需要分配1,000,000字节。
- 然后设置第8,000,000位。<br/>
存在的问题：很明显，集合中只有1个整数，整数占用4个字节，但是Bitmap已经分配了1兆，整整多了6个数量级

**Roaring Bitmap 的工作原理**

Roaring Bitmap是一组无符号整数，由不相交子集的容器组成。每个子集都有一个16位的索引，可以保存大小为 $2^16$ 范围内的值。容器大小的选择还确保在最坏情况下，容器仍然适合现代CPU的L1缓存
...
- 如果是字符串，快速高效判重的方式都是模糊判重。想要精确判重需要记住分而治之的方式。借助离散hash算法，将字符串转换为数字，然后再将数字放入到Roaring Bitmap中。
...
* [布隆过滤器演示](https://www.jasondavies.com/bloomfilter/)
* [Bloom filters Calculator](https://hur.st/bloomfilter/)
* [Bloom Calculator](https://krisives.github.io/bloom-calculator/)

### 2.1、概念

Bloom Filter-布隆过滤器，是一种空间效率很高的随机数据结构，其是由一个固定大小的二进制向量或者位图（bitmap）和一系列（通常好几个）映射函数组成的

原理是：当一个元素被加入集合时，通过K个Hash函数将这个元素映射成一个位阵列（Bit array）中的K个点，把它们置为1。检索时，我们只要看看这些点是不是都是1就（大约）知道集合中有没有它了：
- 如果这些点有任何一个0，则被检索元素一定不在；
- 如果都是1，则被检索元素很可能在；
- 布隆过滤器可以添加元素，但是不能删除元素。为什么不能删除元素呢？原因很简单，因为删除元素后，将对应元素的下标设置为零，可能别的元素的下标也引用改下标，这样别的元素的判断就会收到影响，导致误判率增加；

Bloom Filter的这种高效是有一定代价的：在判断一个元素是否属于某个集合时，有可能会把不属于这个集合的元素误认为属于这个集合（false positive）。因此，Bloom Filter不适合那些“零错误”的应用场合。而在能容忍低错误率的应用场合下， Bloom Filter 通过极少的错误换取了存储空间的极大节省；
...
在Java中一般可以使用`java.util.BitSet`来实现，主要步骤：
...
- 哈希策略：在BloomFilterStrategies枚举中定义了两种哈希策略，都基于著名的MurmurHash算法，分别是`MURMUR128_MITZ_32`和`MURMUR128_MITZ_64`
...
- [Redis-BloomFilter](https://redis.io/docs/latest/develop/data-types/probabilistic/bloom-filter/)

Redis v4.0 之后有了 Module（模块/插件） 功能，Redis Modules 让 Redis 可以使用外部模块扩展其功能，[RedisBloom](https://github.com/RedisBloom/RedisBloom)就是Redis核心的一个扩展模块，用来解决布隆过滤器问题的，这个模块是需要安装的。在更高版本就不需要手动安装了
...
RedisBloom 提供了多种语言的客户端支持，包括：Python、Java、JavaScript 和 PHP，其中Java的支持是[JedisBloom](https://github.com/RedisBloom/JRedisBloom)、[Redisson-BloomFilter](https://github.com/redisson/redisson/blob/78c388ac7ace92ea7fae4d64ffe280c590c0b08b/redisson/src/main/java/org/redisson/api/RBloomFilter.java)

使用 Redission 的 RBloomFilter 示例代码
```java
...
RBloomFilter<String> bloomFilter = Redisson.create(config).getBloomFilter("myBloomFilter");
// 初始化布隆过滤器，定义期望容量和误报率
bloomFilter.tryInit(1000000, 0.01);
// 添加元素
bloomFilter.add(item);
// 是否包含元素
bloomFilter.contains(item)
```
使用 Redission的 RBloomFilter，会根据布隆过滤器名字在Redis中生成两个key，比如上面代码的名字是“myBloomFilter”，生成的配置为：
- "myBloomFilter"是string类型，布隆过滤器的主存，用来存储二进制数组；
- "{myBloomFilter}:config"是hash结构，存储元信息，比如大小size、期望容量expectedInsertions、误报率falseProbability、使用的哈希函数数量hashIterations等。

### 2.4、误判率

- [布隆过滤器误判率推导公式](http://pages.cs.wisc.edu/~cao/papers/summary-cache/node8.html)
...
Counting Bloom Filter：它用一个 counter 数组，替换数组的比特位，这样一比特的空间就被扩大成了一个计数器。

用多占用几倍的存储空间的代价，给 Bloom Filter 增加了删除操作

## 3、布谷鸟过滤器

- [Cuckoo-PDF](https://www.itu.dk/people/pagh/papers/cuckoo-jour.pdf)

### 3.1、布谷鸟Hash

布谷鸟Hash工作原理，总结起来：
- 它有两个 hash 表，记为 T1，T2；两个 hash 函数，记为 h1，h2；
- 当一个不存在的 x 元素插入的时候，会先根据 h1 计算出其在 T1 表的位置，如果该位置为空则可以放进去；
- 如果该位置不为空，则根据 h2 计算出其在 T2 表的位置，如果该位置为空则可以放进去；
- 如果该位置不为空，就把当前位置上的元素踢出去，然后把当前元素放进去就行了；
- 也可以随机踢出两个位置中的一个，总之会有一个元素 y 被踢出去；
- 被踢出去的元素重复上述过程，插入元素 y。
...
- [布谷鸟过滤器：实际上优于布隆过滤器](http://www.linvon.cn/posts/cuckoo/#sec8)
- [布谷鸟过滤器：实际上优于布隆过滤器-原文](https://www.cs.cmu.edu/~dga/papers/cuckoo-conext2014.pdf)
- [布谷鸟过滤器演示过程](http://www.lkozma.net/cuckoo_hashing_visualization/)
- [Redis-Cuckoo BloomFilter](https://redis.io/docs/latest/develop/data-types/probabilistic/cuckoo-filter/)
...
解决的问题：高效处理冲突并支持元素删除，在保证准确性的同时提高数据插入、删除和查询的效率，适用于对操作效率有较高要求且需要处理冲突和支持删除的场景。
...
利用大容量的外存作为中转，将10亿个数切分成小块，每一块排序好后写入外存。
切分完成后，对这些小块进行归并排序。同时在归并排序过程中，获得最大（小）值将实时写入文件，这样就可以保证低内存占用；利用分治策略，利用外存保存中间结果，再用多路归并来排序;

# 八、时间轮-TimeWheel算法

- [时间轮算法](https://mp.weixin.qq.com/s/lBJs8mQpPJGdleLqtGS-dw)
- [时间轮算法简介](https://spongecaptain.cool/post/widget/timingwheel/)
- [时间轮应用场景](https://mp.weixin.qq.com/s/bHFb6AiqM88Uyqt0xBnQzw)
...
- [牛顿迭代法分析过程](https://netedu.xauat.edu.cn/jpkc/netedu/jpkc2009/jsff/content/dzja/3.4.htm)
...
```
上面while循环中可能存在溢出：`squre = middle * middle;`，可以按照如下方法修改：
```java
double middle;
...
```c
// 源码地址： openjdk_8/jdk/src/share/native/java/lang/fdlibm/src/e_sqrt.c
...
或者使用如下方式
```java
static double newtonSqrt(int n, double precision) {
    //当n>=1时，从n开始迭代；
    //当n<1时，从1开始迭代
    double res = Math.max(n, 1);
    while (res * res - n > precision)
        res = 0.5 * (res + n / res);
    return res;
}
```
...
- [加速幂](https://www.desgard.com/algo/docs/part2/ch01/1-quick-pow/)
...
- [数组中滑动窗口实现](../数据结构/数组/数组.md#18长度最小的连续子数组)
- [字符串中滑动窗口实现](字符串/字符串.md#11最长无重复子串)
...
return res;
```
不定长窗口中又可以分为最小滑动窗口和最大滑动窗口
- 最小滑动窗口：给定数组 nums，定义滑窗的左右边界 i, j，求满足某个条件的滑窗的最小长度
  ```java
    while j < len(nums){
        判断[i, j]是否满足条件
        while 满足条件{
  ...
  ```
- 最大滑动窗口：给定数组 nums，定义滑窗的左右边界 i, j，求满足某个条件的滑窗的最大长度
  ```java
    while j < len(nums):{
        判断[i, j]是否满足条件
        while 不满足条件：{
            计算
            i++ （最保守的压缩i，一旦满足条件了就退出压缩i的过程，使得滑窗尽可能的大）
        }
        不断更新结果（注意在while外更新！）
        j++
    }
  ```
> 上述两者关键的区别在于，最大滑窗是在迭代右移右边界的过程中更新结果，而最小滑窗是在迭代右移左边界的过程中更新结果
...
双指针（Two Pointers）：指的是在遍历元素的过程中，不是使用单个指针进行访问，而是使用两个指针进行访问，从而达到相应的目的；双指针分类：
- 对撞指针：即两个指针向相反的方向移动；
- 快慢指针：即两个指针向相同的方法移动；
- 分离指针
...
return 没找到 或 对应值
```
适用范围：一般适合解决有序数组或字符串相关问题
- 查找有序数组中满足某些约束条件的一组元素问题：比如二分查找、数字之和等问题。
- 字符串反转问题：反转字符串、回文数、颠倒二进制等问题

### 6.2、快慢指针

- [关于快慢指针的问题](https://mp.weixin.qq.com/s/MLGauAOe2fpq1d18A69naQ)
...
适用范围：快慢指针一般用于处理数组中的移动、删除元素问题，或者链表中的判断是否有环、长度问题

> 对于链表需要注意的一点是快慢指针对于链表的长度是奇数还是偶数是不一样的：
...
双指针，在做关于单链表的题是也很有用，比如“判断单链表是否有环”、“如何一次遍历就找到链表中间位置节点”、“单链表中倒数第 k 个节点”等问题。对于这种问题，我们就可以使用双指针了，会方便很多。
