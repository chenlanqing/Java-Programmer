...
给你两个非负整数的字符串形式 num1 和 num2，请你计算它们的和，并以字符串形式返回结果。  
示例输入：num1 = "456"，num2 = "3778"  
期望输出："4234"

### 误区

1. 用 Long.parseLong 直接转
2. 用 BigInteger 算不算过关？

### 竖式加法 + 双指针

核心思路一句话：小学算加法怎么算的，代码就怎么写。 从最低位（字符串末尾）开始，逐位相加，算上进位，一位一位往高位推

为什么用双指针？因为两个数不一定一样长，我们要各自从末尾往回走：
1. i 指向 num1 的末尾（最低位）
2. j 指向 num2 的末尾（最低位）
3. 每次取两个字符转成数字相加，加上上一位留下的 carry（进位）
...
**欧几里德算法定理**：两个正整数 a 和 b(a>b) 它们的最大公约数等于 a除以b的余数 c和b 之间的 最大公约数
