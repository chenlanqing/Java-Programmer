...
1.JDK 中的栈:JDK 提供了两种栈
	...
	两种栈都实现了一些方法:
		...
		int size()			取得栈中元素的个数	
2.顺序栈的原理:顺序栈 Stack 继承自 Vector, Vector 和 ArrayList 非常相似,都是动态顺序表
...
三.队列的模拟与使用:
1.JDK 中的队列:JDK 中,将 LinkedList 初始化为 Queue 接口的实例对象,可以实现链式队列的,
...
1.问题描述:
	(1).如何判断一个数字n，是否只含有m这个素数因子? 比如:16只含有2这个素数因子；24除了2之外，还含有3这个素数因子
		解决办法：
