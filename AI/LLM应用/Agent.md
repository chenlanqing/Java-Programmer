...
ReAct 是一种让大模型通过“推理（Reasoning）→ 行动（Acting）→ 观察（Observation）”循环完成任务的 Agent 模式。大模型会根据当前任务和已有信息判断下一步应该采取什么行动，如果需要外部信息，就调用相应工具；工具返回结果后，大模型再根据结果继续推理，并判断是否需要继续调用工具、调整调用参数或进行重试，直到满足任务完成条件，最终生成答案。
