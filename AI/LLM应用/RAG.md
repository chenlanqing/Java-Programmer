...
第一步：把混合检索作为基础设施标准不管用什么上层技术，底层检索层请用Hybrid RAG（BM25+向量+RRF）。这是投入产出比最高的升级，几乎没有理由不做。
...
第四步：评估先行：接入RAGAS或LlamaIndex内置评估套件，建立基线指标（faithfulness、answer relevancy、context recall），再做技术改进。没有指标的优化是盲目的
...
测试集是评估的基础。测试集质量差，后面所有指标都没意义

**原则一：规模要够**

二三十个问题远远不够。比如项目有 5000 份文档，测试集最少 200 条，按文档类型和问题类型均匀分布。一般的经验值是：每 100 份文档至少对应 5-10 条测试问题

**原则二：类型要全**

不能全是"XX 的定义是什么"这种简单事实题。我们把问题分成五类：

| 问题类型     | 占比 | 示例                                       | 考察能力     |
| ------------ | ---- | ------------------------------------------ | ------------ |
| 事实提取     | 30%  | "A 款保险的免赔额是多少？"                 | 精确定位     |
| 多文档综合   | 25%  | "比较 A 款和 B 款保险的保障范围"           | 跨文档整合   |
| 推理判断     | 20%  | "这个理赔案例符合哪款保险的赔付条件？"     | 逻辑推理     |
| 时效性       | 15%  | "最新的车险理赔流程有哪些变化？"           | 时间过滤     |
| 否定/拒答    | 10%  | "帮我算一下这笔理赔金额"                   | 意图识别     |

最后一类很容易被忽略——有些问题知识库里面根本没有答案，正确的回答应该是"我无法回答"。如果测试集里全是知识库能回答的问题，你永远测不出系统在"该拒答时有没有拒答"这个维度的表现。

**原则三：每条问题必须标注三样东西**  
第一，标准答案（ground_truth_answer）：用来评估生成质量。  
第二，标准证据（ground_truth_chunks）：用来评估检索质量。标注的是"回答这个问题需要哪些文档片段"，这样才能计算检索的召回率和准确率。  
第三，元信息：问题类型、难度等级。这样你可以按类型和难度拆分看评估结果，发现"系统在多文档综合题上表现差"或者"困难题准确率明显低"这种模式。  

**原则四：混合构建——人工 + LLM**  
先让 LLM 从文档中自动生成问答对（用 GPT-4 阅读文档片段，生成 QA），大约能覆盖 70% 的测试集。然后人工审核每一条——修正不准确的答案、补充遗漏的证据标注、删除太简单或重复的问题。最后由领域专家手动补充 30% 的高难度问题（多跳推理、时效性、拒答类），这些问题 LLM 自动生成的质量不够
...
需要「A 关联到 B，B 关联到 C，然后从 A 推到 C」的问题，就叫多跳推理（Multi-hop Reasoning）。传统 RAG 在这种场景下经常翻车，给出的答案要么不完整，要么干脆瞎编。信息是分散在多个文档里的，传统 RAG 一次性检索根本搞不定这种链式推理

**2. 全局性问题，它根本答不上**

全局性问题，比如：你问「这本小说的主题思想是什么？」、「这份年度财报里，公司战略发生了哪些变化？」、「这个技术文档集合里，主要讨论的是哪几类方向？」这些问题有一个共同的特点：答案不在某一个文本块里，而是需要你通读所有文档、归纳总结才能得出来；

传统 RAG 的本质是「检索」，它只会找 Top-K 个最像你问题的文本块，大概率只是一些零散的片段，大模型拿到这些片段根本没法总结出整体主题；

本质上不是一个「检索」任务，而是一个「全局归纳」任务

**3. 切块带来的「语义断裂」**

文档切块（Chunking）本身就在毁坏语义
...
> 上面其实讲的都是同一个问题：传统 RAG 做的是「找相似文本」，但企业真正需要的是「理解实体关系」

怎么让 RAG 真正「理解关系」呢？引入知识图谱。这也正是 GraphRAG 的核心思路
...
和传统 RAG 完全一样。把一整本书按一定的字符数（比如 1200 字）切成一个个文本块（GraphRAG 里叫 Text Unit）。

**2. 实体与关系提取（Entity & Relationship Extraction）**
...
**3. 生成实体/关系摘要（Summarization）**

GraphRAG 会把同一个实体在所有文本块里的描述汇总起来，让 LLM 合成一段更完整的实体描述。关系也是同样的处理

**4. 社区检测（Community Detection）**

现在已经有完整的知识图谱了，但是这张图里有成千上万个节点，怎么组织呢？GraphRAG 用的是 Leiden 算法，这是一种层次化社区检测算法，比常见的 Louvain 算法更稳定一些

**5. 生成社区摘要（Community Reports）**

社区划分完之后，GraphRAG 会为每一个社区单独生成一份摘要报告（叫 Community Report）。这一步也是靠 LLM 来做。GraphRAG 会把一个社区内部的所有实体、关系、还有可选的 claim 一股脑儿都喂给 LLM，让它写一份总结报告，这些社区报告就是 GraphRAG 回答全局性问题的核心资产

### 查询阶段

GraphRAG 提供了查询模式：Local Search 和 Global Search 和 DRIFT Search
...
Local Search 的核心是「从一个点扩散到它的邻居」，很适合回答那些「围绕某个具体对象」的问题。
...
本质就是：把全局性问题拆分成多个社区内部的子问题并行解答，最后再汇总

**为什么 Global Search 特别费钱？** Map 阶段要遍历所有相关社区的报告。如果层级选得低（比如 Level 2），可能涉及几百上千个社区，每个都要调一次 LLM 生成中间答案。一次全局查询可能就要消耗上万 token。

## 存在问题

上面这套流程：虽然强大，但步骤多、调用贵、流程复杂，但在实际落地时存在如下问题

**1. 索引成本高得吓人（Token 焚烧炉）**

它每一步几乎都在调 LLM。每个文本块要调 LLM 来抽实体和关系，抽完之后每个实体和关系还要再调 LLM 合成一段综合描述，然后每个社区又要调 LLM 生成一份社区报告，要是你还开了 claim 抽取，那又是一轮 LLM 调用来提取各种断言。

你把这些加起来就会发现，整个过程等于把所有原文通过 LLM 反复读了好几遍，token 消耗轻轻松松就是原文的十倍甚至几十倍

**2. 实体消歧**

实体消歧（实体歧义解析） —— 当文本中提到的名称可能对应多个不同实体时，系统要判断它到底指的是哪一个真实对象，并统一归并到正确节点。 LLM 做实体抽取，天然就是不一致的。同一个实体在不同文档里被叫做不同名字，LLM 抽取出来之后，就变成了好几个独立的节点。
...
解决方案：动态社区选择（Dynamic Community Selection）的优化，从根节点逐层下钻，只展开相关的社区，能把平均社区数从 1500 降到 470 左右

**4. 增量更新，牵一发而动全身**

一旦新增文档，里面可能会冒出一个实体，它其实和图里已有的某个老节点是同一个东西，但名字不一样，于是实体消歧问题又来了。
这种「数据互联性」带来的牵一发而动全身，让 GraphRAG 在动态场景下几乎无法高效维护

## GraphRAG增量更新

### 增量更新的难点

1. 第一步的麻烦，新文档里的实体可能和现有图谱里的实体需要合并。
2. 第二步的麻烦，新的实体和关系会改变图的结构：图结构变了，意味着什么？原来的社区划分可能过时了。可能新加入的一堆实体本应该属于某个新社区，或者老社区的边界发生了变化
3. 第三步的麻烦，社区一变，下游的社区摘要就全失效了。
4. 第四步的麻烦，向量索引也要同步更新：新的实体、关系、社区摘要都要重新向量化，存进向量库

传统 RAG 里一步就能搞定的事情，在 GraphRAG 里变成了一连串「牵一发而动全身」的级联操作。

### 应对增量更新的思路

**1. 思路一：简单粗暴，定期全量重建**：最土但最省心的办法：每天、每周、或者每月定时把所有文档重新跑一遍 GraphRAG 索引。
...
**2. 思路二：增量索引（Incremental Indexing）**

支持增量索引，只处理新增的文档，把新抽出的实体和关系追加到已有图上
...
- 缺点：精度会有损失。因为社区划分本质是一个全局优化问题，只重做局部社区，全局的社区划分就不一定是最优的。时间长了，图谱的质量会慢慢「漂移」，需要阶段性做一次全量重建来「校准」。

**3. 思路三：混合策略，小增量 + 周期性全量**

生产常见的实现方案：就是把前面两个思路结合起来用。日常有新文档进来，就跑一次增量索引，又快又便宜；但每隔一段时间（比如每个月一次），再做一次全量重建，把之前累积漂移掉的精度拉回来。

这种方式的优点就是成本和质量的平衡最好，缺点嘛，就是你得同时维护两套流程

**4. 时间分区（Temporal Partitioning）**
...
索引时不做消歧，只把所有实体直接加进去，哪怕同一个 IBM 有四个节点也无所谓。然后查询时再做合并，或者异步离线跑消歧任务。
- 优点：索引极快，增量几乎零成本。
...
GraphRAG 的增量问题，本质上是「社区摘要」这个设计带来的。那问题来了：能不能干脆不要社区摘要，用别的方式实现全局查询呢？
...
GraphRAG 两个局限：
- 一个是现有基于图的 RAG 方法（主要就是指微软 GraphRAG）缺乏动态更新能力，很难高效处理新信息的加入，数据一变整个图谱就得大改；
- 另一个是全局查询的「暴力搜索」实在太低效，GraphRAG 的 Global Search 是 Map-Reduce 遍历所有社区摘要的，在大规模语料上又慢又贵

LightRAG 的目标，就是同时解决这两个问题。而它解决问题的方式，是引入了三个核心创新：

**1. 创新一：图增强的文本索引（Graph-Enhanced Text Indexing）**

LightRAG 和 GraphRAG 一样，也是用 LLM 从文档里抽实体和关系，构建知识图谱。但它做得更轻，其不做社区检测，它只保留实体节点 + 关系边这个最核心的图结构，然后把实体和关系的描述都向量化，存进向量数据库

**2. 创新二：双层检索范式（Dual-Level Retrieval）**

不做社区摘要了，那全局性问题怎么答？LightRAG 的做法是：不用提前做全局聚合，而是在查询时把查询本身拆成两层关键词，分别做检索
...
**对比 GraphRAG:**
牺牲了「全局深度洞察」，LightRAG 的双层检索虽然轻量，但本质还是基于查询去「现场拼凑」 全局视角，对极其复杂的全局归纳问题，深度可能不如 GraphRAG。

所以两者的权衡其实很清楚。如果你追求的是极致的全局深度洞察，而且预算管够，那 GraphRAG 是你的选择；反过来，如果你更在意的是成本可控、增量友好、实时性好，愿意在全局洞察深度上打个折扣，那 LightRAG 就更合适

## 与 GraphRAG

根据【文档内容】，以下是 GraphRAG 与 LightRAG 在适用场景与核心差异方面的完整对比总结。

### **GraphRAG 的适用场景**

GraphRAG 是“深度分析场景的重型武器”，它设计复杂、成本高，但在特定场景下能提供其他方案无法替代的深度和精度。它主要适用于以下几类场景：

**场景一：需要“全局洞察”的深度内容分析**  
这是 GraphRAG 最核心的主场。其预先生成的社区摘要能够对全局数据进行高层次归纳，解决传统 RAG 无法回答的宏观问题。
* **典型示例**：
    * **长篇文学作品的主题分析**：如分析一本50万字小说的核心思想、人物关系网络、社会冲突模式。
    * **多篇研究报告的综述**：如“过去十年这个行业的主要发展脉络是什么？”、“这50份白皮书里主流观点的演进趋势是什么？”
    * **大规模专利/学术论文分析**：如“过去五年某个疾病领域的研究热点是如何演进的”，这需要在数万篇文献上进行主题发现和演进追踪。

**场景二：跨文档多跳推理的高价值场景**  
这类问题的回答需要精确关联多个文档中的实体关系，且一旦答错代价高昂。GraphRAG 的图遍历能力能够完整地走通复杂的逻辑链路。
* **典型示例**：
    * **合规与审计追踪**：如“哪些贷款客户关联的公司最近被监管点名，且在我行的信贷额度超过5000万？”，需要打通“客户-公司关联-监管记录-信贷档案”四类数据。
    * **医疗诊断辅助**：如“糖尿病患者合并高血压，使用ACEI类药物的禁忌症有哪些？”，需要准确追溯“疾病-药物-禁忌症”的关系链。
    * **法律案例检索**：如“过去类似的合同争议在不同司法管辖区是如何判决的？”，需要关联“案件-判决-法规-判例引用”多层关系。
    * **金融研报深度分析**：如“A公司的哪些高管最近有变动，其中有没有出现在竞争对手公司的前任高管名单里？”

**场景三：数据相对静态、更新不频繁的知识库**  
由于GraphRAG的增量更新成本极高，它最适合那些建好索引后能长期服务、更新周期很长的静态知识库。
* **典型示例**：企业内部的规章制度库、历史档案、法律条文库、已归档的项目资料、公司的年度研究报告集。这类数据可能几个月甚至一年才更新一次，初始构建成本可以被长期查询收益摊薄。

**场景四：对精度和可解释性要求极高的场景**  
GraphRAG 的图谱结构（节点-边-社区）是显式、可追溯的，这使得其答案具备极强的可解释性和可审计性。
* **典型示例**：金融合规决策、医疗诊断证据支撑、法律咨询建议、政府政务回答等，这些场景要求每个结论都必须有据可查，能够追溯到具体的原始证据。

**不适用GraphRAG的场景**：数据量小（如<10万token）、业务变化快且数据每日更新、预算紧张的初创项目、C端实时交互产品（无法忍受高延迟），以及仅需单跳事实问答的业务（如客服FAQ）。

### **LightRAG 的适用场景**

LightRAG 是一款“轻量化、工程友好型”的GraphRAG替代方案，其定位是满足那些“想吃图RAG的红利，但又扛不住GraphRAG成本和维护负担”的需求。它主要适用于以下几类场景：

**场景一：数据频繁更新的业务**
这是 LightRAG 最核心的主场。其“追加式”增量更新设计，使得新数据能够以近乎零成本的方式快速整合进系统。
* **典型示例**：
    * **企业知识库的日常维护**：每天都有新产品说明、客户反馈、项目总结等文档加入。
    * **新闻聚合和实时资讯**：如金融新闻分析系统，需要分钟级地纳入最新新闻。
    * **客户支持系统**：CRM中每天新增的工单、投诉、解决方案记录。
    * **持续更新的技术文档**：如GitHub项目的API文档、内部技术Wiki，几乎每天都在变化。

**场景二：成本敏感的中小团队和初创公司**
LightRAG 将索引成本降低了约99%，让预算有限的团队也能用上图RAG的能力。
* **典型示例**：个人知识管理、教育/学习辅助AI助手、SaaS产品的多租户知识库功能。

**场景三：实时性要求高的C端应用**
LightRAG 的查询延迟在秒级，能够满足C端产品对即时交互体验的要求。
* **典型示例**：客服聊天机器人、智能编程/客户助手、语音交互应用。用户无法接受GraphRAG Global Search那种长达数十秒的等待。

**场景四：轻量部署、本地化环境**
LightRAG 架构轻量，资源消耗低，可以在普通硬件或严格的内网环境中运行。
* **典型示例**：
    * **边缘设备部署**：在工厂车间的边缘盒子上部署，帮助工人查询技术手册。
    * **私有化内网部署**：金融、医疗、政府等对数据隐私要求高的行业，可以完全离线运行。
    * **本地个人AI助手**：在个人笔记本上使用Ollama等工具运行本地大模型，配合LightRAG构建个人知识库。

**场景五：中小规模知识库（10万-500万token）**
大多数企业的内部知识库数据量都落在这个区间，这是LightRAG的“甜蜜区”。对于超过5000万token的超大规模语料，GraphRAG的社区摘要价值会更大。

**LightRAG的代价**：在需要**极深度全局洞察**（例如，总结一本巨著的终极哲学主题）的场景下，其“按需检索、现场组装”的方式可能不及GraphRAG预计算的社区摘要深入。同时，其对时效性冲突（如新旧信息矛盾）的处理能力较弱。

### 核心维度完整对比

| 对比维度 | GraphRAG（微软） | LightRAG（港大） |
| :--- | :--- | :--- |
| **设计哲学** | **预计算全局视角**：在索引阶段通过社区摘要“嚼碎”全局结构。 | **按需检索，延迟聚合**：索引阶段只做核心抽取，查询时临时组装视角。 |
| **核心创新** | 社区检测（Leiden算法） + 社区摘要生成。 | 双层检索（Dual-Level Retrieval） + 增量友好设计。 |
| **索引阶段步骤** | 5步：文档切块 → 实体关系抽取 → 实体/关系摘要 → **社区检测** → **社区摘要生成**。 | 3步：文档切块 → 实体关系抽取 → 键值对生成与去重。**无社区相关步骤**。 |
| **查询机制** | • **Local Search**：从入口实体图遍历。<br>• **Global Search**：**Map-Reduce式遍历社区摘要**（慢，成本高）。 | • **Hybrid模式（默认）**：用LLM将问题拆为**低层**（实体）与**高层**（关系）关键词，并行检索后合并。<br>• 另有Naive, Local, Global模式。 |
| **索引成本** | **极高**（“Token焚烧炉”）。构建100万token索引约需20-50美元。 | **极低**。构建100万token索引仅需约0.15美元，**降低约99%**。 |
| **查询延迟** | **慢**。Global Search端到端延迟通常在10秒至1分钟。 | **快**。Hybrid查询延迟在1-3秒，适合实时交互。 |
| **增量更新** | **困难且成本高**。新文档可能触发社区重划、摘要重建等级联反应，维护复杂。 | **简单且成本低**。直接“追加”新实体和关系到图谱，几乎零额外成本。 |
| **实体消歧** | 需要复杂的多层策略（字符串+向量+LLM判别），精度高但实现贵。 | 采用朴素的**名称匹配**去重，实现简单，但可能遗留语义相似的多个节点。 |
| **深度全局洞察** | **强**。预生成的社区摘要能提供深层次的宏观归纳。 | **一般**。依靠查询时临时拼凑，在极深度全局归纳问题上可能不及GraphRAG。 |
| **工程复杂度** | 高。 | 低。 |
| **本质比喻** | 深度分析的**“重型武器”**。 | 日常使用的**“轻巧刀具”**。 |

### 选型决策总结

![](../image/GraphRAG与LightRAG决策树.png)

1.  **起步验证**：先用**传统RAG**搭建MVP，观察实际需求。
2.  **优先升级**：若遇到多跳推理、全局归纳问题，优先考虑**LightRAG**，因其在成本、速度和可维护性上优势显著。
3.  **重型备用**：仅当LightRAG无法满足**极深度全局洞察** 或 **极端精度要求**，且预算和延迟可接受时，才选用**GraphRAG**。
4.  **混合架构**：对于大型复杂系统，可考虑用GraphRAG处理静态核心知识，用LightRAG处理动态增量数据，通过查询路由器分流。

GraphRAG 是深度分析的重型武器，LightRAG 是日常使用的轻巧刀具。大多数企业的 RAG 落地其实用 LightRAG 就能覆盖 80% 的需求，剩下 20% 的深度分析需求再上 GraphRAG 做补充，这可能是目前最务实的选型策略
...
- [Sirchmunk：无需向量数据库和预索引的自进化搜索引擎](https://github.com/modelscope/sirchmunk)