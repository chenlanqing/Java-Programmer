...
提示注入防御的核心思想是：Agent 读取的配置文件、MCP 工具输出、搜索结果等，都不能被当作天然可信的指令，而应该被当作需要审查的外部“代码”。工程上可靠的做法是组合使用多层机制：通过权限限制工具能力（比如前面介绍的 Hooks ）；通过“允许列表”限制可访问域名或命令；通过 Sandbox 隔离执行环境；通过人工审批（HITL）处理高风险操作等。
