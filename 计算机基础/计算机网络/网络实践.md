...
WebRTC（Web Real-Time Communication）是一套开放标准和 API 集合，允许浏览器和移动应用在不需要中间服务器的情况下，实现点对点（P2P）的音视频流和数据传输。由 Google 主导，2011 年开源，现已成为 W3C 和 IETF 标准，2011 年开始推进，最初目的就是让浏览器之间能直接做实时音视频通话，不需要安装任何插件。它最核心的设计决策是把底层从 TCP 换成 UDP。
...
WebRTC 更像是一套「协议全家桶」，在 UDP 之上叠加了好几层协议，每层各管一件事，组合在一起才能完成实时音视频通信；

![](image/WebRTC-协议栈-多次协议组合.png)
