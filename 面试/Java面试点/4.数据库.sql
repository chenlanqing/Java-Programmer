...
6.数据库中的索引的结构有哪些结构,索引底层是如何实现的?
	...
	6.2.B-树(B树): -- http://blog.jobbole.com/111757/
		6.2.1.数据库索引为什么使用树结构存储?
			(1).树的查询效率高;
		...
		6.2.2.B树是一种多路平衡查找树,它的每一个节点最多包含k 个孩子,k 称为 B树的阶.k的大小取决于磁盘页的大小
			一个m阶的B树具有以下特征:
			(1).根节点至少有两个子女
			...
			例子: "附件/3阶B树.jpg"
		6.2.3.B树的查询:B 树的查询比较次数其实不比二叉搜索树少,尤其是当单一节点中的元素数量很多时.
		...
		6.2.6.B树主要应用于文件系统以及部分数据库索引,如非关系型数据库 MongoDB.	
	6.3.B+树:基于B-树的一种变体.
		6.3.1.一个m阶的B+树具有如下几个特征：
			...
			例子:"附件/3阶B+树.jpg"
		6.3.2.在图片(3阶B+树.jpg)中,根节点元素 8 是子节点2,5,8的最大元素,也是叶子节点6,8的最大元素.
			==> 根节点的最大元素等同于整个B+树的最大元素,无论插入删除多少元素,始终保持最大元素在根节点中.
		6.3.3.由于父节点的所有元素都出现在子节点,因此所有叶子节点包含了全量的元素信息,并且每一个叶子节点
			都带有指向下一个节点的指针,形成了一个有序链表.
		6.3.4.卫星数据:B+树还有一个在索引外的特点,却是至关重要的特点,卫星数据的位置.
		...
		6.3.7.大部分关系型数据库,比如Mysql都是用B+树作为索引的
...
14.数据库水平拆分(分表)和垂直拆分(分库)? 分库/分表中主键问题? 
	...
	14.1.数据切分(Sharing):基本思想是把一个数据库切成多个部分放到不同的数据库上,从而缓解单一数据库的性能问题.
