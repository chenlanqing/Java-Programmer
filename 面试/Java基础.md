...
## Collectors 类提供了很多现成的收集器，有没有办法实现自定义的收集器
...
### 为什么 JDK8 的 HashMap 使用的跟以往不同的实现
...
### 为什么HashMap的默认初始容量是16，且容量必须是 2的幂
...
### 如果HashMap在put的时候，如果数组已有某个key，不想覆盖怎么办？取值时，如果得到的value是空时，如何返回默认值；
...
### 使用ConcurrentHashMap中，如何避免组合操作的线程安全问题
...
## 哪些map的key可以为null、value可以为null？为什么
...
## DirectBuffer 与 MappedByteBuffer
...
### threadpoolexecutor的内部数据结构是什么样子的
...
### Spring中有哪些和ThreadPoolExecutor类似的工具
...
详细参考：[线程池异常处理](../Java/源码分析/thread/线程池.md#310线程池异常)

### 如何实现优先开启更多的线程，而把队列当成一个后备方案呢
...
### 工作中线程池提交有两种方式，execute和submit；他们的区别是什么，在使用的时候，需要注意什么问题

### 使用线程池的时候，并且程序运行起来了，那么有什么办法能够在不停止服务的情况下改变这个core size

setCoreSize

### 线上一个核心交易服务使用线程池处理请求，在高并发场景下偶发接口超时

排查思路：
1. 通过监控面板或 Arthas 查看 ThreadPoolExecutor 的 activeCount、poolSize、queueSize、completedTaskCount、rejectCount 等运行时指标；
2. 分析任务耗时分布（P50/P99/P999）和排队时间占比，区分是任务本身变慢还是排队等待导致超时；
3. 检查线程栈（jstack/Arthas thread）是否存在线程阻塞在 IO、锁等待、GC 上；
4. 结合系统指标（CPU 使用率、GC 频率和耗时、网络 RT）排除外部因素

### 线程池调优

- 队列容量应根据 SLA 和任务耗时计算：队列长度 ≈ SLA允许的等待时间 / 平均任务耗时
- 拒绝策略选择：关键交易链路用 CallerRunsPolicy 施加反压，非核心链路用 DiscardPolicy 快速降级
- 推荐动态线程池：通过配置中心（Nacos/Apollo）运行时调整 corePoolSize、maxPoolSize、queueCapacity，无需重启服务（如美团线程池框架、Hippo4j）
- 参数验证：通过全链路压测逐步调整，观察 QPS、RT、CPU、GC 等指标的变化拐点

### 如果线程池队列使用的是无界队列，在流量洪峰时可能导致什么严重后果？如何设计合理的拒绝策略和熔断降级方案？

一、无界队列的严重后果：
1. 任务无限堆积：LinkedBlockingQueue 默认容量 Integer.MAX_VALUE，流量洪峰时任务持续入队无法拒绝
2. 延迟持续恶化：队列中等待任务越多，排队时间越长，P99/P999 急剧上升，最终触发上游超时重试，形成流量放大
3. 内存风险：每个 Runnable 任务对象占用堆内存，大量堆积导致 Old Gen 膨胀，频繁 FullGC 甚至 OOM
4. maximumPoolSize 失效：ThreadPoolExecutor 的扩容逻辑是'核心线程满→入队→队列满→扩容到最大线程→拒绝'，无界队列永远不满，最大线程数形同虚设
5. 线程泄漏：如果任务中持有上下文信息（如 ThreadLocal、TraceId），堆积的任务还可能导致内存泄漏

二、合理的拒绝策略设计：
- 有界队列容量 = 峰值QPS × 可容忍等待时间 / 单任务处理时间
- 自定义 RejectedExecutionHandler：
• 核心交易：记录告警日志 + CallerRunsPolicy 施加反压，迫使上游减速
• 非核心业务：降级返回默认值或缓存数据
• 可异步业务：转入 MQ 延后处理

三、熔断降级方案：
- 接入 Sentinel/Resilience4j，线程池维度配置熔断规则：当拒绝率 > 阈值或 RT P99 > SLA 时触发熔断
- 线程池隔离：不同业务使用独立线程池，避免相互影响
- 限流前置：在网关层（如 Nginx/Spring Cloud Gateway）做令牌桶或滑动窗口限流，从源头控制流量
- 降级策略：熔断期间返回兜底数据（缓存/默认值），而非直接报错
- 超时控制：任务执行设置硬超时（Future.get(timeout)），防止个别慢任务拖垮整个线程池
...
## synchronized 无法禁止指令重排序，确能够保证有序性？
...
**同步队列：**
同步队列主要用于实现锁的获取和释放。如常用的ReentrantLock，就是基于同步队列来实现的。

它是一个FIFO队列，节点类型为AQS内部的Node类。当一个线程尝试获取锁失败时，它会被封装成一个Node节点加入到队列的尾部(每个节点(Node)代表一个等待的线程)。当锁被释放时，头节点的线程会被唤醒，尝试再次获取锁。
...
- 使用方式不同:同步队列是AQS自动管理的，开发者通常不需要直接与之交互;而条件队列是通过Condition接口暴露给开发者的，需要显式地调用等待(await)和通知(signal/signalAlI)方法。
...
AQS 使用的是一个 **CLH 队列的变体，本质上是 FIFO 双向链表**。使用双向链表主要是为了同时支持：

1. **通过 `prev` 找到前驱节点**
   线程获取锁时，需要判断自己的前驱是否是头节点：
...
只有前驱是 `head` 的节点才有资格竞争锁，这保证了同步队列整体上的 FIFO 顺序。

2. **通过 `next` 唤醒后继节点**
   线程释放锁时，需要找到头节点的后继节点，并调用 `LockSupport.unpark()` 唤醒它：
...
双向链表可以直接定位前驱和后继，降低节点摘除和队列修复的复杂度。

> AQS 使用双向链表，不只是为了实现 FIFO，更重要的是线程获取锁时需要通过 `prev` 判断前驱节点，释放锁时需要通过 `next` 唤醒后继节点；同时线程中断、超时取消时，还需要快速跳过或移除取消节点。双向链表能够同时支持前驱查找、后继唤醒和取消节点清理。

AQS 中通常是：

* `prev` 链路相对可靠，入队时会明确设置；
* `next` 链路可能因为并发入队或节点取消而暂时不完整；
* 因此唤醒后继节点时，如果 `next` 不可用，AQS 可能从 `tail` 开始通过 `prev` 反向查找有效节点。

所以双向链表还有一个作用：**在并发修改导致正向链路不可靠时，可以通过反向链路进行兜底查找。**


### 为什么有了 CAS 还要 volatile

CAS和 volatile在AQS中是互补的:
- CAS提供原子性操作以避免锁的使用；
...
### LinkedBlockingQueue 的双向队列与 SynchronousQueue 的双向队列有什么区别
...
## ThreadLocalRandom可以把它的实例设置到静态变量中，在多线程情况下重用吗？
...
## Synchronized 会发生进程间的上下文切换吗？具体又会发生在哪些环节呢？
...
**悲观锁**：假定会发生并发冲突，因此在操作数据之前通过锁机制阻止其他事务对该数据的访问。常用方式是 `SELECT ... FOR UPDATE`，适合写操作频繁、冲突概率高的场景
...
## LongAddr 和 AtomicLong 有什么区别？

LongAdder是Java 8中推出了一个新的类，主要是为了解决AtomicLong在多线程竞争激烈的情况下性能并不高的问题。它主要是采用分段+CAS的方式来提升原子操作的性能。

相比于AtomicLong，LongAdder有更好的性能，但是LongAdder是典型的以空间换时间的实现方式，所以所需要用到的空间更大，而且LongAdder可能存在结果不准确的问题，而AtomicLong并不会。

**LongAddr 为什么结果会不准确？**

在没有并发更新的情况下sum方法将返回准确的结果，但是在计算总和时发生的并发更新可能不会被计算进来，所以就有可能会不准确。
...
- ThreadPoolExecutor 是基于任务分配 (Task-Assignment)算法实现的线程池，ThreadPoolExecutor 中线程池中有一个共享的工作队列，所有任务都将提交到这个队列中。线程池中的线程会从队列中获取任务执行，如果队列为空，则线程会等待，直到队列中有任务为止。
    ...
    ForkJoinPool 中的任务通常是一些可以分割成多个子任务的任务，例如快速排序。每个任务都可以分成两个或多个子任务，然后由不同的线程来执行这些子任务。在这个过程中，ForkJoinPool会自动管理任务的执行、分割和合并，从而实现任务的动态分配和最优化执行。

    ForkJoinPool中的工作线程是一种特殊的线程，与普通线程池中的工作线程有所不同。它们会自动地创建和销毁，以及自动地管理线程的数量和调度。这种方式可以降低线程池的管理成本，提高线程的利用率和并行度。

    ThreadPoolExecutor中线程的创建和销毁是静态的，线程池创建后会预先创建一定数量的线程，根据任务的数量动态调整线程的利用率，不会销毁线程。如果线程长时间处于空闲状态，可能会占用过多的资源。

在使用场景上也有区别：
- ThreadPoolExecutor 适合处理I0密集型或普通CPU任务，如网络请求处理、数据库访问、Web服务请求调度。尤其是大量独立、不需要拆分的小任务。
- ForkJoinPool 合于 CPU密集型、可拆分的并行计算任务，
    1. 大任务分解为小任务:适用于可以递归分解为更小任务的大型任务。ForkJoinPool通过分而治之的方式，将大任务拆分为小任务，这些小任务可以并行处理。
    2. 计算密集型任务:对于需要大量计算且能够并行化的任务，ForkJoinPool是一个理想的选择。它能够有效利用多核处理器的优势来加速处理过程。
    3. 递归算法的并行化:适合于可以用递归方法解决的问题，如快速排序、归并排序、图像处理中的分区算法等。4.数据聚合任务:在处理需要聚合多个数据源结果的任务时(例如，遍历树结构并聚合结果)，ForkJoinPool提供了有效的方式来并行化这一过程。

## 为什么 CompletableFuture 使用 ForkJoinPool

CompletableFuture使用 ForkJoinPool 而不是 ExecutorService 的原因主要是因为它的执行模型和任务分割方式与ForkJoinPool 更加匹配。

在CompletableFuture中，一个任务可以分割成多个子任务，并且这些子任务之间可以存在依赖关系。而 ForkJoinPool 本身就是一种支持任务分割和合并的线程池实现，能够自动地处理任务的拆分和合并。而且，ForkJoinPool 还有一种工作窃取算法，能够自动地调整线程的负载，提高线程的利用率和并行度。

ForkJoinPool还有一个特点，就是它的线程池大小是动态调整的。当任务比较少时，线程池的大小会自动缩小，从而减少了线程的数量和占用的系统资源。当任务比较多时，线程池的大小会自动增加，从而保证任务能够及时地得到执行。

如果使用ExecutorService来执行这些任务，需要手动地创建线程池、任务队列和任务执行策略，并且需要手动地处理任务的拆分和合并，实现起来相对比较复杂。

## Thread.sleep(0) 有什么用途？
...
### 虚拟线程为什么不建议使用 synchronized

虚拟线程是在JEP425中提出的，但是JEP425在介绍他的用法时，特意提到了一个比较关键的情况，那就是虚拟线程在两种情况下可能会被PINNED。(JDK24中的 JEP491开始，也不再PINNED了)
...
但是，这个问题，在25年3月18日 JDK 24推出之后，已经有所改变了。在 JEP 491Synchronize VirtualThreads without Pinning中做了优化, 不再进行绑定了.(https://openjdk.org/jeps/491)
...
还有就是，虚拟线程的引入有个目的是为了简化并发编程模型，让开发者可以像编写顺序代码一样编写并发代码而无需担心线程管理和线程池的复杂性。要求虚拟线程不被池化，是为了鼓励开发者利用这一简化的模型，避免回到传统的线程池管理模式。

### 虚拟线程为什么不建议使用 ThreadLocal

虚拟线程是支持ThreadLocal的，但由于可能创建的虚拟线程数量巨大，不当使用ThreadLocal可能导致内存泄漏等问题。如果需要，可以考虑使用作用域局部变量(Scope-local variables)作为替代方案。

ThreadLocal 本身不是不能用，而是不建议滥用。它主要有几个问题：
- 第一，ThreadLocalMap 的 key 是弱引用而 value 是强引用，如果在线程池中忘记 remove，可能造成内存泄漏；
- 第二，线程池会复用线程，如果清理不及时可能造成用户、租户或 TraceId 等上下文串数据；
- 第三，ThreadLocal 属于隐式参数，会让方法之间的数据依赖不透明；第四，在虚拟线程场景下线程数量可能非常大，如果通过 ThreadLocal 缓存大对象，会造成严重的内存放大。

替代方案要看场景。普通业务参数优先显式参数传递；对于 TraceId、UserContext、TenantContext 这种只读上下文，从 JDK 25 开始可以使用正式版 ScopedValue。它采用作用域绑定，退出作用域后自动失效，不需要手动 remove，并且在大量虚拟线程场景下时间和空间成本更低。

但 ScopedValue 并不是 ThreadLocal 的完全替代，如果确实需要线程级可变状态，ThreadLocal 仍然有使用价值。

## new Thread().start() 启动线程，操作系统层面上发生了什么？

new Thread() 只是创建 Java Thread 对象，真正调用 start() 后，HotSpot JVM 会进入 native 层，通过 pthread 等系统线程 API 请求操作系统创建原生线程；Linux 内核为其创建 task_struct、线程栈和调度上下文，并加入运行队列，由 OS 调度器调度 CPU 执行。传统 Java 平台线程与 OS 线程基本是 1:1 映射，因此创建大量线程会产生线程栈、内核资源和上下文切换成本

```mermaid
flowchart TB
    A[new Thread] --> B[创建 Java Thread 对象]
    B --> C[start]
    C --> D[JVM start0 native]
    D --> E[创建 OS 原生线程]
    E --> F[pthread_create]
    F --> G[clone 系统调用]
    G --> H[内核创建 task_struct]
    H --> I[分配线程栈等资源]
    I --> J[加入调度队列]
    J --> K[CPU 调度执行]
    K --> L[JVM 调用 Thread.run]
```
...
## 进程分配内存不够时向Linux申请内存时，Linux系统如何处理

首先，进程申请内存时，比如调用 malloc，并不一定会立即分配对应的物理内存。用户态的 glibc 会根据情况通过 brk 或 mmap 向 Linux 申请虚拟地址空间。

Linux 使用虚拟内存和按需分页机制，进程拿到虚拟地址后，第一次真正访问这块内存时，如果页表中还没有对应的物理页，就会触发 Page Fault。内核处理 Page Fault，分配物理页并建立虚拟地址到物理地址的映射，然后重新执行原来的指令。

如果此时物理内存不足，Linux 会尝试回收可回收内存，例如 Page Cache；如果配置了 Swap，还可能把部分不活跃页面换出到 Swap，从而释放物理内存。

如果经过内存回收和 Swap 后仍然无法满足内存需求，就可能触发 OOM Killer，根据 OOM 策略选择进程杀掉，释放其占用的内存。

所以整个过程可以概括为：malloc → brk/mmap → 虚拟地址 → Page Fault → 分配物理页 → 内存不足 → 回收/Swap → OOM

### 为什么 malloc 后不立即分配物理内存？

因为 Linux 使用虚拟内存和 Demand Paging。申请内存和实际使用内存是两个阶段。这样可以避免进程申请了大量内存但实际上没有使用时，提前浪费物理内存。

## JDK7、8、9 默认垃圾收集器分别是什么？
...
### Java 8和Java11的GC有什么区别?

首先,在垃圾收集器上面，Java8 中默认的Parallel Scavenge GC + Parallel Old GC的，分别用来做新生代和年代的垃圾回收。而在 Java11 中默认采用的是G1进行整堆回收女的(Java9中就是默认的了)。

另外,Java11中还新增了一种垃圾收集器,那就是ZGC,他可可以在保证高吞吐量的同时保证最短的暂停时间。

在垃圾识别及回收上面,Java8基于的是单纯地可达性分析,i而Java11中的G1采用的是三色标记法,可以大大降低STW的时长。

另外,G1的内存划分是自适应的，它会根据堆的大小和使用情况来动态调整各个区域的大小和比例。而Parallel ScavengeGC + Parallel Old GC都是固定分配的策略。
...
1. 如果JIT的逃逸分析后该对象没有逃逸，那么可能优化到栈上上分配。
2. 否则对象主要分配到新生代上，如果启动了TLAB，则分配到到TLAB中。
3. 如果被判断为大对象，则直接分配到直接进入老年代，譬如很长的字符串和数组，避免为大对象分配内存时由于分配担保机制带来的复制而降低效率。可以设置 -XX:PretenureSizeThreshold，令大于该尺寸的对象直接进入老年代；

所以，我们到这里就很清楚了，当给对象分配内存的时候，有可能在栈上分配，这自然不存在线程安全问题。除此之外，如果在堆上分配，则可能会启动TLAB机制，使得堆内存给线程单独划分空间，避免了线程安全的问题。同时，当不启动TLAB机制的时候，如果一个空间被多个线程同时分配对象，JVM会采用CAS+失败重试的方式来避免线程问题。简而言之，是采用乐观锁的方式，只有假定该堆没有被其他线程操作的时候，当前线程才会在堆上分配对象，如果被其他线程操作，就获取当前堆中的最新标识，然后重试

## Java 虚拟机中的堆一定是共享的吗？

从 JVM 规范来看，堆是所有线程共享的运行时数据区，主要用于存放对象实例和数组。

但是 HotSpot 等具体 JVM 实现为了提高对象分配效率，会在堆的 Eden 区中为每个线程划分 TLAB。线程可以优先在自己的 TLAB 中进行对象分配，因此从分配实现角度看，堆中存在一部分线程私有的分配区域。

但需要注意，TLAB 的线程私有性不等于对象的线程私有性。通过 TLAB 创建出来的对象仍然属于共享堆，其他线程只要获得对象引用，就可以访问它。

堆是共享的，TLAB 是线程私有的；TLAB 中创建的对象仍然属于共享堆。

## Full GC多久一次算正常？

其实，Full GC的频率取决于多个因素，包括应用程序的性质、堆的大小、内存分配和释放的模式等。正常情况下，Full GC应该是相对较少发生的，因为频繁的Full GC会导致应用程序的性能下降和响应时间延长。

如果一定要给一个指标，那么可以给一个经验值，拿我们这面一个非常核心的应用来说：这个应用日常的QPS在5000以上，线上一共有100台左右的机器
