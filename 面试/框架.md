...
## 34、SpringBoot Starter

### 1. Starter 和普通 jar 的核心区别是什么？

**核心区别不在于技术形式，而在于“职责定位”**。

| 维度           | 普通 jar      | Spring Boot Starter |
| ------------ | ----------- | ------------------- |
| 主要职责         | 提供功能代码      | 提供“自动装配能力”          |
| 是否包含业务逻辑     | 是           | 通常不直接包含或极少          |
| 是否参与 Bean 注册 | 不主动         | 主动进行自动配置            |
| 与 Spring 耦合度 | 可选          | 强依赖 Spring Boot     |
| 使用方式         | 显式引入 + 手动配置 | 依赖即生效（约定优于配置）       |

**一句话总结：**

> 普通 jar 解决“能不能用”，Starter 解决“用起来是否无感、是否标准化”。

---

### 2. Spring Boot 是如何发现并加载 Starter 的？

#### （1）Starter 的发现机制

Spring Boot **并不是识别 Starter 名称**，而是依赖以下链路：

```text
Starter 依赖
 → 引入 AutoConfiguration jar
 → META-INF/spring.factories（或新版本 spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports）
 → SpringFactoriesLoader / AutoConfigurationImportSelector
 → 自动装配类
```

#### （2）核心流程（以 Spring Boot 2.x 为例）

1. Spring Boot 启动时
2. `@SpringBootApplication`
   → `@EnableAutoConfiguration`
3. `AutoConfigurationImportSelector`
4. 从 **classpath 中所有 jar** 读取：

   ```
   META-INF/spring.factories
   ```
5. 加载 `EnableAutoConfiguration` 对应的配置类
6. 根据 `@Conditional` 条件判断是否生效

#### （3）本质总结

> Starter ≠ 自动生效
> **AutoConfiguration + Conditional 才是关键**

---

### 3. 如果用户自己定义了 Bean，你的 Starter 如何处理？
...
| 场景         | 解决方式                        |
| ---------- | --------------------------- |
| 用户自定义 Bean | `@ConditionalOnMissingBean` |
| 用户想替换部分实现  | 提供接口 + 默认实现                 |
| 用户想扩展逻辑    | 提供 SPI / 回调接口               |
| 用户想完全接管    | 提供 enabled=false            |

#### 面试可用总结

> Starter 的原则是：**兜底，而不是抢占**

---

### 4. Starter 为什么一定要有 enabled 开关？

#### 原因一：Starter 默认是“隐式生效”的
...
* Bean 冲突
* 资源初始化（线程池、连接池）
* 改变系统行为（Filter、Interceptor、生效顺序）
...
> enabled 是 Starter 的“熔断器”和“最小侵入保障”。

---

### 5. 如何避免 Starter 对业务产生侵入？
...
> 一个好的 Starter，业务代码“感觉不到它的存在”。

---

### 6. 你在写 Starter 时遇到过什么问题？

可以从以下几个“真实工程痛点”中选择回答：
...
> Starter 最大的难点不是功能实现，而是 **边界控制和条件设计**。

---

### 7. Starter 组件中的 spring.factories 文件是否有必要存在？
...
com.xxx.starter.XxxAutoConfiguration
```
...
> 在 Spring Boot 2.x 中是必须的，它是 Starter 被发现的核心入口；
> 在 Spring Boot 3.x 中虽然不再使用 spring.factories，但依然需要等价的自动装配声明文件。

---

### 一句话总总结（非常适合收尾）

> Starter 的本质不是“封装功能”，而是 **以最小侵入的方式，把一组最佳实践自动化交付给使用者**。
...
3. **检查Aware**
- 如果Bean实现了`BeanNameAware`、`BeanClassLoaderAware`等这些Aware接口，Spring容器会调用它们。
- 在`AbstractAutowireCapableBeanFactory`的`initializeBean`方法中调用

4. **调用BeanPostProcessor的前置处理方法**
- 在Bean初始化之前，允许自定义的`BeanPostProcessor`对Bean实例进行处理，如修改Bean的状态。`BeanPostProcessor`的`postProcessBeforeInitialization`方法会在此时被调用。
...
- 提供一个机会，在所有Bean属性设置完成后进行初始化操作。如果Bean实现了`InitializingBean`接口，`afterPropertiesSet`方法会被调用。
...
- 在Bean初始化之后，再次允许`BeanPostProcessor`对Bean进行处理。`BeanPostProcessor`的`postProcessAfterInitialization`方法会在此时被调用。
...
- 当容器关闭时，如果Bean实现了`DisposableBean`接口，`destroy`方法会被调用。
...
> 简化流程顺序记忆：
实例化 → 填充属性 → Aware接口 → BeanPostProcessor前置 → `afterPropertiesSet()` → `init‑method` → BeanPostProcessor后置 → Bean就绪 → 容器关闭 → `destroy()` → `destroy‑method`

### SpringBean的初始化过程

Spring Bean 的初始化过程主要包括：
1. **实例化 Bean**：Spring 通过反射调用构造方法创建 Bean 实例。
2. **属性填充（依赖注入）**：给 Bean 的成员变量注入依赖，比如 `@Autowired`、`@Resource`。
3. **Aware 接口回调**：如果 Bean 实现了 `BeanNameAware`、`BeanFactoryAware` 等接口，Spring 会回调对应方法，让 Bean 获取容器相关信息。
...
6. **BeanPostProcessor 后置处理**
   ...
   * AOP 动态代理通常在这里生成。
7. **Bean 放入单例池**：初始化完成后放入 `singletonObjects`，后续直接获取。

Spring Bean 初始化过程主要分为实例化、属性填充、初始化和销毁几个阶段。创建 Bean 时首先根据 BeanDefinition 实例化对象，然后进行依赖注入，接着执行各种 Aware 回调。之后经过 BeanPostProcessor 前置处理，执行 @PostConstruct、InitializingBean 和 init-method 初始化方法，再经过 BeanPostProcessor 后置处理生成 AOP 代理，最后放入单例池 singletonObjects 中。Spring 通过三级缓存解决单例 Bean 的循环依赖问题。

### 什么情况下 Spring Bean 会初始化失败？

Bean 初始化失败主要发生在生命周期阶段。
- 第一是实例化阶段，例如构造方法异常、没有合适构造函数；
- 第二是依赖注入阶段，例如依赖 Bean 不存在、多 Bean 冲突、循环依赖；
- 第三是初始化阶段，例如 @PostConstruct、afterPropertiesSet、init-method 执行异常；
- 第四是基础设施问题，例如数据库、Redis连接失败；
- 第五是框架代理阶段，例如 AOP BeanPostProcessor 创建失败。
- 另外生产环境中最常见的是配置错误、依赖冲突和外部资源不可用。

Spring Boot 启动失败时，本质都是 ApplicationContext.refresh() 过程中 BeanFactory 创建单例 Bean 失败，最终抛出 BeanCreationException。
...
1. 构造器注入循环依赖：构造器在实例化阶段调用，此时 Bean 尚未创建完成，无法提前暴露 ObjectFactory。解决方案：@Lazy 注入代理对象，延迟到首次使用时初始化
2. Prototype Bean 循环依赖：Prototype 每次都创建新实例，Spring 不会缓存 Prototype Bean，因此三级缓存机制不适用
3. `@Async` 标注的 Bean 循环依赖：异步代理通过 AsyncAnnotationBeanPostProcessor 的 postProcessAfterInitialization 创建，不在 getEarlyBeanReference 阶段处理，可能导致循环依赖时注入的是非异步代理对象。需 @Lazy 解决
4. 多例与单例混合循环：Prototype A 依赖 Singleton B，B 又依赖 Prototype A，每次获取 A 都是新实例，无法解决

### SpringBoot 2.6+默认禁止了循环依赖，如果你接手的遗留项目存在大量循环依赖，你会如何系统性重构？有哪些实践原则？

核心原则就是：不以“让 Spring 能启动”为目标，而是以“消除错误的业务依赖关系”为目标

```mermaid
flowchart TD
    A[开启临时兼容配置]-->B[扫描并建立依赖图]
    B-->C[识别所有循环依赖]
    C-->D{循环类型}
    D-->E[简单字段依赖]
    D-->F[Service双向依赖]
...
一、分析阶段：
1. 启用临时配置 spring.main.allow-circular-references=true 使项目能启动
2. 通过 Spring Boot 启动日志中的 BeanCreationException 识别直接循环依赖；
3. 使用 IntelliJ IDEA 的 Analyze Dependencies 或 Spring Bean Dependency Analyzer 插件生成 Bean 依赖图
4. 静态代码分析：扫描 @Autowired/@Resource 注入关系，识别直接循环（A↔B）、间接循环（A→B→C→A）和跨模块循环
5. 运行时分析：通过 Actuator 的 /beans 端点查看实际 Bean 依赖关系

二、分类重构策略：
1. 简单字段循环依赖（A↔B 直接依赖）：
- 检查职责边界：A 是否真的需要 B，还是只需要 B 中的某个方法？提取该方法到独立组件 C，A→C←B
- 调整依赖方向：如 Controller→Service→Repository，避免 Service→Controller 的反向依赖
2. 跨服务双向调用（Service A↔Service B）：
- 提取公共领域服务：将 A 和 B 共同依赖的逻辑提取为 Service C，A→C←B
- 引入中介者模式（Mediator）：创建 Mediator 服务协调 A 和 B 的交互
- 事件驱动解耦：A 调用 B 时改为发布领域事件，B 监听事件并处理，消除 A→B 的直接依赖
3. 跨模块循环依赖：
- 依赖倒置原则（DIP）：定义公共接口模块，A 和 B 均依赖接口而非实现
- 模块重划：识别错误的模块边界，按领域驱动设计（DDD）重新划分限界上下文
- 引入 Facade 或 API 层隔离模块间直接调用
4. 同类内部方法调用导致的'伪循环依赖'：
- 将互调方法提取到独立 Bean，消除自注入
...
```
2. 集成到 CI/CD 流水线，每次提交自动验证
3. 关闭 allow-circular-references，新增循环依赖将导致启动失败
4. Code Review 中增加依赖关系检查项
5. 定期审查 Bean 依赖图，防止架构腐化

四、核心原则：
- 循环依赖是设计问题的症状，不是 Spring 的缺陷
- @Lazy 是创可贴不是解药，掩盖了设计问题
- 重构应渐进式：先加测试保障→逐个解除循环→验证→关闭开关
- 最终目标：建立清晰的模块依赖方向（无环有向图 DAG）
...
在Spring框架中，使用`@PostConstruct`、自定义的`init‑method`方法，和`InitializingBean`接口和`afterPropertiesSet`方法都是用于在Bean初始化阶段执行特定方法的方式。他们的执行顺序是：**构造函数 > @PostConstruct > afterPropertiesSet > init‑method**
- `@PostConstruct` 是`javax.annotation`包中的注解(Spring Boot 3.0之后`jakarta.annotation`中)，用于在构造函数执行完毕并且依赖注入完成后执行特定的初始化方法。标注在方法上，表示这个方法将在Bean初始化阶段被调用。
...
WebFlux 出现的核心原因是传统 Spring MVC 基于 Servlet 阻塞模型，在高并发 IO 场景下存在线程资源浪费问题。随着微服务、网关、实时通信的发展，单纯依靠增加线程无法线性提升吞吐，因此 Spring 5 引入基于 Reactor 的响应式编程模型，通过非阻塞 IO 提升系统并发能力。
...
## Reactor 非阻塞模型如何解决 C10K？

C10K：单机同时处理 10000 个连接的问题。

WebFlux优势主要在高并发IO密集型场景，如果业务大量依赖阻塞资源，例如传统 JDBC，那么收益有限。

## Spring Boot 和传统双亲委派有什么区别？

Spring Boot 并没有改变 JVM 的双亲委派模型，本质仍然遵循 ClassLoader 的父子委托机制。区别在于 Spring Boot 使用了 LaunchedURLClassLoader，通过自定义 Jar 加载机制支持 Fat Jar，将 BOOT-INF/classes 和 BOOT-INF/lib 中的内容加载到 JVM 中。它解决的是依赖打包和加载路径问题，而不是改变类加载规则。

真正破坏双亲委派的是一些容器场景，例如 Tomcat 的 WebAppClassLoader 使用双亲委派的变体 child-first，实现应用之间依赖隔离。

## SpringBoot 版本升级

Spring Boot 2.0 解决的是传统Java服务高并发IO问题，通过WebFlux和Reactor引入响应式模型，同时升级HikariCP提升数据库连接性能。  
Spring Boot 3.0 完成Spring生态现代化升级，核心是Java17、Jakarta迁移以及AOT和Observability体系建设。  
Spring Boot 4.0进一步拥抱现代Java和云原生，通过Virtual Thread、AOT优化以及Spring AI生态，更适合构建下一代Agent和企业智能应用。  

## Spring MVC 如何实现流式输出？

Spring MVC 可以通过 StreamingResponseBody、ResponseBodyEmitter 和 SseEmitter 实现流式响应。核心原理是利用 Servlet 3.0 异步请求机制，让 Controller 快速返回一个异步响应对象，然后由后台线程持续向 HttpServletResponse 的 OutputStream 写数据，并通过 flush 将数据分块发送给客户端。AI 大模型流式输出一般采用 SSE，通过 text/event-stream 持续推送 token 数据。

另外还可以利用 Flux响应式编程来实现；
