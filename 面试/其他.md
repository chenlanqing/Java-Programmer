...
## 作为 Team Leader 如何安排工作？

如果我是 Team Leader，我一般不会简单地把需求拆完之后平均分给每个人，而是会从目标、人员和风险三个方面来安排。

第一步是明确目标。拿到一个需求以后，我会先和产品、业务确认目标、范围、优先级以及上线时间，特别是一些核心业务规则和验收标准，避免团队一开始理解就出现偏差。

第二步是任务拆解。我会把大的需求拆成几个相对独立的模块，比如接口、核心业务逻辑、数据库、缓存、MQ以及前端联调等，同时识别里面的技术难点和依赖关系。

第三步是人员匹配。不同成员的技术能力和经验是不一样的。比较核心、风险高的模块，我一般会安排经验比较丰富的人负责；相对独立或者风险较低的模块，可以安排给成长中的同学，同时我会给他明确的目标和必要的技术支持。这样既保证项目进度，也能培养团队成员。

第四步是确定计划和里程碑。我会把任务拆成几个关键节点，而不是等到最终交付的时候才检查。例如方案评审、开发完成、联调、测试、上线，每个阶段都有明确的负责人和时间节点。

第五步是过程管理。日常不会事无巨细地盯着每个人，而是重点关注进度、风险和阻塞问题。比如通过站会或者项目看板了解哪些任务延期、哪些任务存在技术风险。如果发现某个成员被问题卡住，我会及时介入，必要的时候协调其他人一起解决。

最后是复盘。项目上线以后，我会关注实际结果，包括有没有线上问题、延期原因、技术债以及团队协作上的问题，然后做一次复盘，把经验沉淀下来。

所以我理解 Team Leader 的核心不是“自己做最多的事情”，而是合理分配资源、提前识别风险、解决团队成员解决不了的问题，最终保证项目按质量和时间要求交付。

## 具体是怎么分配任务的

一般会综合考虑三个因素：任务难度、人员能力和项目风险。高风险任务优先保证项目结果，低风险任务可以作为成员成长机会。

## 如何排期？

我评估需求排期，一般会先做需求拆解，然后再进行工作量评估，而不是拿到一个需求直接估一个总工期。

首先，我会确认需求范围和验收标准，明确哪些是本次必须做的，哪些属于后续优化，避免需求边界不清导致排期失真。

然后把需求拆成具体任务，比如技术方案、数据库改造、后端开发、缓存和 MQ、接口联调、测试、上线等。对于比较复杂的任务，我会继续拆到比较细的粒度，一般希望单个任务能够控制在 0.5～2 天左右，这样估算会更准确。

接下来评估每个任务的工作量，同时考虑人员的实际情况。比如一个熟悉业务的高级开发和一个刚接触业务的开发，同一个任务的实际耗时可能是不一样的。

除了开发工作量，我还会重点评估依赖和风险，比如是否依赖其他团队、是否涉及数据库变更、是否需要第三方接口、有没有历史代码或者技术方案不确定的问题。

最后会把开发、联调、测试以及上线准备时间都考虑进去，并预留一定的风险缓冲。

所以我的排期不是简单地说“这个需求需要 10 天”，而是会拆成：
需求分析和技术方案 + 开发 + Code Review + 联调 + 测试修复 + 上线准备 + 风险缓冲。

如果是团队项目，我还会根据任务之间的依赖关系安排并行开发，最后形成整体排期。

另外，对于不确定性比较大的需求，我不会直接给一个非常精确的时间，而是先安排技术预研或者 PoC，把最大的风险验证掉之后再给正式排期。

## 怎么保证排期比较准确

排期准确的关键不是把每个任务估到非常精确，而是把不确定性提前暴露出来。

比如一个需求涉及第三方支付接口，我不会直接按照接口文档估两天，因为真正的风险可能在联调、异常场景和支付回调。如果这个依赖没有确认，我会把它作为排期风险单独标出来，甚至提前做技术验证。

评估排期时除了估算人天，还会看任务依赖关系和关键路径，尽可能让团队并行开发，最终按照关键路径计算项目周期，而不是简单把所有人的工作量相加。
...
- [面试指导手册](https://github.com/nas5w/interview-guide)
