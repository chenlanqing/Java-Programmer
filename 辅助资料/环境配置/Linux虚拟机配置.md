...
```sh
ssh -N -L 18789:127.0.0.1:18789 root@<you remote ip>
```
通过 SSH 连接远程服务器，同时把远程的 127.0.0.1:18789 映射到你本地的 18789 端口
...
- 常用于端口转发 / 隧道场景
- 否则默认会给你一个远程终端
2. `-L`: 本地端口转发（Local Forwarding），一般格式：
```sh
-L [本地端口]:[远程地址]:[远程端口]
```
| 部分          | 解释                  |
