...
sequenceDiagram
    ...
    participant App as Mobile App(已登录)
    participant Auth as Auth Service

    %% ===============================
    %% 1. 获取二维码
    %% ===============================
    Browser->>Web: GET /api/login/qrcode

    Web->>Redis: 生成 qr_token<br/>status=WAIT_SCAN<br/>expire=300s

    Redis-->>Web: OK

    Web-->>Browser: 200 OK<br/>{qr_token,qrcode_url}

    Note over Browser: 显示二维码

    %% ===============================
    %% 2. 建立状态监听
    %% ===============================
    Browser->>Web: WebSocket /ws/login/{qr_token}
    Web-->>Browser: Connected

    %% ===============================
    %% 3. 手机扫码
    %% ===============================
    App->>Web: POST /api/login/qrcode/scan
    Note right of App: Authorization: Bearer AccessToken<br/>Body:{qr_token}

    Web->>Auth: 校验AccessToken

    Auth-->>Web: user_id=10001

    Web->>Redis: 查询二维码

    Redis-->>Web: status=WAIT_SCAN

    Web->>Redis: 更新状态<br/>status=SCANNED<br/>bind user_id

    Redis-->>Web: OK

    Web-->>App: 200 已扫码

    Web-->>Browser: WS Push<br/>status=SCANNED

    Note over Browser: 页面提示：请在手机确认登录

    %% ===============================
    %% 4. 手机确认登录
    %% ===============================
    App->>Web: POST /api/login/qrcode/confirm
    Note right of App: Authorization: Bearer AccessToken<br/>Body:{qr_token}
    ...
    Redis-->>Web: status=SCANNED

    Web->>Auth: 创建Web Session

    Auth-->>Web: session_id

    Web->>Redis: 更新状态<br/>CONFIRMED
    ...
    %% 5. 通知PC登录成功
    %% ===============================
    Web-->>Browser: WS Push<br/>status=CONFIRMED<br/>session_id

    Browser->>Web: GET /api/login/result?qr_token=xxx

    Web->>Redis: 查询二维码绑定用户

    Redis-->>Web: user_id

    Web-->>Browser: Set-Cookie: SESSION=xxxx

    Note over Browser: 浏览器保存Cookie

    %% ===============================
    %% 6. 正常访问业务接口
    %% ===============================
    Browser->>Web: GET /api/user/profile
    Note right of Browser: Cookie: SESSION=xxxx
...
Value:
{
    ...
    "userId":null,
    "sessionId":null,
    "expireTime":"2026-07-02T12:00:00"
}
...
| 接口                          | Method    | 说明          |
| --------------------------- | --------- | ----------- |
| `/api/login/qrcode`         | GET       | 获取二维码       |
| `/ws/login/{qr_token}`      | WebSocket | 建立二维码状态监听   |
| `/api/login/qrcode/scan`    | POST      | 手机扫码        |
| `/api/login/qrcode/confirm` | POST      | 手机确认登录      |
| `/api/login/result`         | GET       | PC 获取最终登录结果 |
| `/api/user/profile`         | GET       | 登录后的业务接口    |

websocket 可以采用轮询的方式

## 3、微信/支付宝扫码

微信/支付宝扫码登录，与普通扫码登录最大的区别在于：PC 网站并不知道用户是谁，而是通过 OAuth 2.0/OpenID Connect，让微信/支付宝作为身份提供方（IdP，Identity Provider）完成用户认证。
...
sequenceDiagram
    ...
    participant App as 微信/支付宝App
    participant OAuth as 微信/支付宝OAuth

    %% ==========================
    %% 1. PC申请二维码
    %% ==========================

    Browser->>Biz: GET /api/oauth/qrcode

    Biz->>Cache: 生成 login_token<br/>status=WAIT_SCAN

    Cache-->>Biz: OK

    Biz->>OAuth: GET /connect/qrconnect<br/>appid+state=login_token

    OAuth-->>Biz: 返回二维码URL

    Biz-->>Browser: 二维码图片

    Browser->>WS: 建立 WebSocket(login_token)
    ...
    OAuth->>Biz: 回调通知<br/>GET /oauth/callback?code=xxx&state=login_token

    %% ==========================
    %% 4. 服务端换Token
    %% ==========================

    Biz->>OAuth: POST /oauth/access_token
    Note right of Biz: appid<br/>secret<br/>code

    OAuth-->>Biz: access_token<br/>openid

    Biz->>OAuth: GET /userinfo

    OAuth-->>Biz: 用户信息(openid,nickname...)

    %% ==========================
    %% 5. 创建本地登录
    %% ==========================

    Biz->>Biz: 查询openid是否已绑定账号

    alt 已绑定
        Biz->>Biz: 创建Web Session
    else 首次登录
        Biz->>Biz: 注册或绑定账号
        Biz->>Biz: 创建Web Session
    end

    Biz->>Cache: status=CONFIRMED<br/>保存session
    ...
    Biz-->>WS: Push LOGIN_SUCCESS

    WS-->>Browser: 登录成功

    Browser->>Biz: GET /api/oauth/result?login_token=xxx

    Biz-->>Browser: Set-Cookie SESSION=xxxx

    Browser->>Biz: GET /api/user/profile
...
- [通过网站图标进行浏览器指纹识别](https://github.com/jonasstrehle/supercookie/)
...
* [一个基于 SWT 的 API，用于管理用户和发行 SWT 令牌](https://github.com/netlify/gotrue)